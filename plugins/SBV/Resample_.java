...
//		double scaleFactorY = newHeight / (double) height;
    ...
    for (int x = 0; x < newWidth; x++) {
      for (int y = 0; y < newHeight; y++) {
        ...
        double newX = x
            / scaleFactorX; // coord in input image A utilizing backward mapping. forward mapping works only when
        // shrinking the size
        ...
        int resultVal = getNNInterpolatedValue(inDataArrInt,
            ...
            height,
            newX,
            newY); // forward mapping: use x/y coords instead of newX/newY
