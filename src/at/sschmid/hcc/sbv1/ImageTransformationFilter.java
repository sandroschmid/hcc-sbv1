...
//		int[] histogram = new int[] { 100, 50, 0, 5, 100 };
		int expectation = (int) Math.floor(Arrays.stream(histogram).sum() / histogram.length);
...
//		System.out.println();
	...
	public static int[] GetHistogramEqualizationTF2(int maxVal, int[][] inImg, int width, int height) {
		...
		int[] histogram = GetHistogram(maxVal, inImg, width, height);
		int expectation = (int) Math.floor((Arrays.stream(histogram).sum() - histogram[0]) / (histogram.length - 1));	
...
//		probabilities[i] = aMax;
	 ...
	 * https://stackoverflow.com/questions/32015287/histogram-equalization-using-java
	 */
	...
	/**
	 * https://naushadsblog.wordpress.com/2014/02/10/image-processing-and-computer-vision-in-java-point-operators-part-2histogram-equalization/
	 */
