...
public class ECG {
    ...
    public void process() {
        try (final OpenSignalsTextFile ostFile = new OpenSignalsTextFile(this.fileName)) {
            ostFile.open();
            restoreFromFile(ostFile);
//      System.out.println(ostFile.getDevices()
//          .get(0)
//          .getMeasuringPoints()
//          .stream()
//          .map(mp -> String.valueOf(mp[mp.length - 1]))
//          .collect(Collectors.joining("; ")));
            final int[] mps = predictedBaselineCorrection(ostFile);
            System.out.println(Arrays.stream(mps).mapToObj(String::valueOf).collect(Collectors.joining("; ")));
    ...
    private void restoreFromFile(OpenSignalsTextFile ostFile) throws FileNotFoundException {
        final List<OpenSignalsTextFile.Device> devices = ostFile.getDevices();
        for (final OpenSignalsTextFile.Device d : devices) {
            ostFile.read(d.getName());
            LOGGER.info("Found " + d.getMeasuringPoints().size() + " measuring points for device '" + d.getName() + "'");
        }
    }

    private int[] predictedBaselineCorrection(OpenSignalsTextFile ostFile) {
        final OpenSignalsTextFile.Device device = ostFile.getDevices().get(0);
