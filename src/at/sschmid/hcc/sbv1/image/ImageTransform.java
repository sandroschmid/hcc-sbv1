...
public class ImageTransform {
	...
	public ImageTransform(int[][] img, int width, int height) {
		this(new Image(img, width, height));
	}

	public ImageTransform(final Image image) {
		this.image = image;
		this.result = new Image(new int[image.width][image.height], image.width, image.height);
	}

	public ImageTransform transform(final Transformation factor) {
		final double cosTheta = Math.cos(Math.toRadians(factor.rotation));
		final double sinTheta = Math.sin(Math.toRadians(factor.rotation));
...
//				double posY = y - heightHalf;
				...
				posX -= factor.translationX;
				posY -= factor.translationY;
